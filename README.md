...
```
ANDROID_SDK_ROOT=~/Library/Android/sdk ANDROID_NDK_ROOT=~/Library/Android/sdk/ndk/25.2.9519653 ./android.sh --enable-gpl --enable-x264 --enable-libwebp --enable-opus --disable-x86 --disable-x86-64 --disable-arm-v7a
```

> [!IMPORTANT]
> Google request developers to build with new r27 and r28 ndk but ffmpeg-kit has its own NDK compatibility table which supports only upto r25.
...
```
./ios.sh --enable-gpl --enable-x264 --enable-libwebp --enable-opus --disable-armv7 --disable-armv7s --disable-i386 --disable-x86-64 --disable-x86-64-mac-catalyst --disable-arm64-mac-catalyst --xcframework --target=12.1\
```
